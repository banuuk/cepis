...
public class ReportProviderImpl implements ReportProvider {
	protected static Logger log = Logger.getLogger(ReportProviderImpl.class
			.getName());
	...
	/**
	 * @uml.property name="hibernateProvider"
	 * @uml.associationEnd
	 */
	private HibernateProvider hibernateProvider;

	public ReportProviderImpl(DirectoryProvider directoryProvider,
			HibernateProvider hibernateProvider) throws ProviderException {
	...
	public List<Report> getReports() throws ProviderException {
		String fromClause = "from org.efs.openreports.objects.Report report order by report.name ";
	...
	public void deleteReport(Report report) throws ProviderException {
		...
		try {
			...
			iterator = session.createQuery(
					"from org.efs.openreports.objects.ReportGroup reportGroup")
					.iterate();
			...
			if (he.getCause() != null
					&& he.getCause().getMessage() != null
					&& he.getCause().getMessage().toUpperCase()
							.indexOf("CONSTRAINT") > 0) {
