...
dhtmlXTabBar.prototype = {
	_get_size : function(name, alter) {
		var size = this.entBox.getAttribute(name)
				|| this.entBox.style[name]
				|| (window.getComputedStyle ? window.getComputedStyle(
						this.entBox, null)[name]
						: (this.entBox.currentStyle ? this.entBox.currentStyle[name]
								: 0))
		if ((size || "").indexOf("%") != -1)
	...
	setSkin : function(name) {
		...
		if (name.indexOf("dhx_sky") == 0) {
			...
			var f = function() {
				...
				var w = this._tabAll[this._s._vMode ? "offsetHeight"
						: "offsetWidth"]
						+ (_quirks ? 2 : 0);
				if (this._lastActive)
					w = Math
							.max(
									w,
									this._lastActive.parentNode[this._s._vMode ? "scrollHeight"
											: "scrollWidth"]);
				if (w < 6)
					return;
				d1.style[this._py] = parseInt(this._conZone.style[this._py])
						- 3 + "px";
				d1.style[this._dx] = w - 2 + "px";
				d2.style[this._py] = parseInt(this._conZone.style[this._py])
						- 3 + "px";
				d2.style[this._dx] = w - (_quirks ? 6 : 4)
						+ (this._s.expand ? 2 : 0) + "px";
				d3.style[this._dy] = parseInt(this._tabZone.style[this._dy])
						- 3 + "px";
	...
	_setSizes : function(x, y) {
		...
		var _t = this._rows.length
				* (this._s.line_height - (this._s.skin_line_x ? 4 : 2))
				+ (this._s.skin_line_x ? 2 : 0);
		this._tabZone.style[this._dy] = _t + "px";
		this._conZone.style[this._dy] = Math.max(0, _h - _t - 2
				- (this._s.skin_line_x ? 3 : 0) - this._s.tab_margin)
				+ "px";
		this._conZone.style[this._dx] = Math.max(0, _w
				- (this._s.expand ? 0 : 2))
				+ "px";
		this._tabZone.style[this._py] = this._s.tab_margin + "px";
		this._conZone.style[this._py] = _t + this._s.tab_margin
				- this._s.skin_line + "px";
		this._checkScroll();
	...
	_showScroll : function(i) {
		...
		a[0].style.cssText = "background-image:url("
				+ this.imgUrl
				+ this.skin
				+ "/"
				+ this.skin
		...
		a[1].style.cssText = "background-image:url("
				+ this.imgUrl
				+ this.skin
				+ "/"
				+ this.skin
	...
	_setRowSizes : function() {
		for ( var i = 0; i < this._rows.length; i++) {
			...
			this._rows[i].style[this._py] = i
					* (this._s.line_height - (this._s.skin_line_x ? 4 : 2))
					- ((_isIE && !window.postMessage && this._s._bMode) ? 0 : 0)
					+ "px";
	...
	_setTabSizes : function(row) {
		...
		for ( var i = 0; i < row.tabCount; i++) {
			...
			if (tab.style.display == "none")
				continue;
			tab.style[px] = pos
					- (this._lastActive == tab ? this._a.select_shift : 0)
					+ "px";
	...
	addTab : function(id, text, size, position, row) {
		...
		if ((!position) && (position !== 0))
			position = z;
		var tab = this._createTab(id, text, (size == "*" ? 10 : size
				|| this._s._tabSize));
	...
	_createTab : function(id, text, size) {
		...
		if (this._close)
			thml += "<IMG style='" + this._pxc + ":4px;" + this._py
					+ ":4px;position:absolute;' src='" + this.imgUrl
					+ this.skin + "/close.png' >"
		tab.innerHTML = thml;
	...
	_img_all : function(tab) {
		...
		this._img(tab.childNodes[3], pf + 1, this._px,
				parseInt(tab.style[this._dx]) - (a.left + a.right), a.left)
	},
	_get_img_pos : function(ind) {
		if (this._s._bMode && ind < 7)
			ind = Math.abs(ind - 6);
		ind = -5 - ind * 45;
		if (this._s._vMode)
			return ind + "px 0px";
	...
	_img : function(tag, y, pos, a, b) {
		tag.style.backgroundImage = "url(" + this.imgUrl + this.skin + "/"
				+ this.skin + "_" + this._s.mode + ".gif)";
	...
	_addTab : function(row, tab, size, position) {
		...
		if (row.tabCount) {
			...
			var pos = parseInt(last.style[this._s.align ? this._pxc : this._px])
					+ parseInt(last._size) + this._a.margin
		}
		;
	...
	adjustTabSize : function(tab, size) {
		...
		if (!size) {
			...
			size = tab.scrollWidth + 10 + (this._close ? 20 : 0);
			tab.style.fontWeight = ""
		}
		;
		tab.style[this._dx] = size + "px";
		tab._size = size;
		this._img(tab.childNodes[3], 5, this._px, size - (a.left + a.right),
				a.left)
	},
	...
	_showHover : function(tab) {
		if (tab == this._lastHower)
			return;
		if (this._lastHower && this._lastHower != this._lastActive) {
			var a = this._getSkin(this._lastHower);
			this._lastHower.className = this._lastHower.className.replace(
					/[ ]*dhx_tab_hover/gi, "");
			if (a.hover)
				this._img_all(this._lastHower);
			else
				this._img(this._lastHower.childNodes[3], 5, this._px,
						parseInt(this._lastHower.style[this._dx])
								- (a.left + a.right), a.left);
			this._lastHower = null
		}
		;
		if (tab && (tab == this._lastActive || tab._disabled))
			return;
		this._lastHower = tab;
		if (tab) {
			var a = this._getSkin(tab);
			...
			;
			this._img(tab.childNodes[3], 0, this._px,
					parseInt(tab.style[this._dx]) - (a.left + a.right), a.left)
		}
	},
	...
	_onClickHandler : function(e) {
		...
		if (target && !target._disabled)
			...
			if (src.className == "dhx_tab_scroll_left") {
				src.parentNode.scrollLeft = Math.max(0,
						src.parentNode.scrollLeft - src.parentNode.offsetWidth
								/ 2);
				tag = src;
			...
			if (tag) {
				...
				if (!_isIE || window.XMLHttpRequest)
					tag.nextSibling.style.right = tag.parentNode.scrollLeft
							* (-1) + "px";
				return false
			}
		}
	...
	_setTabActive : function(tab, mode) {
		if (!tab || tab == this._lastActive)
			return true;
		var id = tab.getAttribute("tab_id");
		var last_id = this._lastActive ? this._lastActive
				.getAttribute("tab_id") : null;
		var a = this._getSkin(tab);
		if (!mode)
			if (!this.callEvent("onSelect", [ id, last_id ]))
				return;
		var px = this._s.align ? this._pxc : this._px;
		this._setTabInActive(this._lastActive);
	...
	_scrollTo : function(tab) {
		if (!this._s.scrolls)
			return;
		if (tab.offsetLeft < tab.parentNode.scrollLeft
				|| (tab.offsetLeft + tab.offsetWidth) > (tab.parentNode.scrollLeft + tab.parentNode.offsetWidth)) {
			tab.parentNode.scrollLeft = tab.offsetLeft;
			var tag = tab.parentNode._scroll;
			if (tag && tag[0]) {
				tag[0].style.left = tag[0].parentNode.scrollLeft + "px";
				if (!_isIE || window.XMLHttpRequest)
					tag[1].style.right = tag[1].parentNode.scrollLeft * (-1)
							+ "px"
			}
		}
	},
	_setTabTop : function(tab) {
		...
		for ( var i = 0; i < this._rows.length; i++)
			if (this._rows[i] == tab.parentNode) {
				...
				var wh = (this._s._vMode ? Math.max(this._tabZone.offsetHeight,
						row.scrollHeight) : Math.max(this._tabZone.offsetWidth,
						row.scrollWidth));
				if (wh > 0)
	...
	_getCoverLine : function() {
		if (!this._lineA) {
			...
			this._lineA.style[this._py] = this._s.line_height - 3
					- (this._s.skin_line_x ? 1 : 0) + "px";
		...
		if (!this._content[id]) {
			...
			d.show = function() {
				...
				if (self._awdj || self._ahdj) {
					...
					if (self._ahdj)
						self._tabAll.style.height = self.entBox.style.height = dim[1]
								+ self._rows.length
								* (self._s.line_height - 2)
								+ (self._s.expand ? 0 : 2) + 2 + "px";
					self._setSizes();
			...
			d.hide = function() {
				...
				if (self._hrfmode == "iframe")
					this.attachURL("javascript:false")
			};
			...
			d._doOnAttachURL = function() {
				...
				self.callEvent("onTabContentLoaded", [ this
						.getAttribute("tab_id") ])
			};
	...
	loadXMLString : function(xmlString, call) {
		this.XMLLoader = new dtmlXMLLoaderObject(this._parseXML, this, true,
				this.no_cashe);
	...
	_parseXML : function(that, a, b, c, obj) {
		...
		that._a.margin = parseInt(
				(atop.getAttribute("margin") || that._a.margin), 10);
		acs = atop.getAttribute("align");
		if (acs)
			that._s.align = (acs == "right" || acs == "bottom");
		that._a.offset = parseInt(
				(atop.getAttribute("offset") || that._a.offset), 10);
		acs = atop.getAttribute("skinColors");
		if (acs)
			that.setSkinColors(acs.split(",")[0], acs.split(",")[1]);
		for ( var i = 0; i < arows.length; i++) {
			...
			for ( var j = 0; j < atabs.length; j++) {
				...
				if (atabs[j].getAttribute("selected"))
					selected = id;
				if (that._hrfmode)
	...
	enableContentZone : function(mode) {
		this._conZone.style.display = ((convertStringToBoolean(mode)) ? ""
				: "none");
		this._setSizes()
	},
	...
	setSkinColors : function(a, b) {
		if (a)
			this._a.tab_color = a;
		if (b)
			this._a.data_color = b;
		this._conZone.style.backgroundColor = b
	},
	...
	setCustomStyle : function(id, color, scolor, style) {
		...
		if (this._tabs[id])
			this._tabs[id].childNodes[0].style.cssText = ((this._tabs[id] == this._lastActive) ? this._c[id].scolor
					: this._c[id].color)
	},
	...
	getNext : function(tab, alt) {
		alt = alt || "nextSibling";
		var next = tab[alt];
		if (next && next.className.indexOf("dhx_tab_element") == -1)
			next = null;
	...
	goToNextTab : function(tab) {
		do {
			tab = this.getNext(tab || this._lastActive)
		} while (!this._setTabActive(tab))
	},

	goToPrevTab : function(tab) {
		do {
			tab = this.getNext((tab || this._lastActive), "previousSibling")
		} while (!this._setTabActive(tab))
	},
	...
	tabWindow : function(tab_id) {
		return (this._content[tab_id] ? this._content[tab_id]._frame.contentWindow
				: null)
	},
	...
	setContentHref : function(id, href) {
		...
		case "ajax":
			...
			cell.attachHTMLString = function(str, xml) {
				if (xml)
					str = that._getXMLContent(xml.doXPath("//content")[0]);
	...
	normalize : function(limit, full) {
		...
		for ( var j = 0; j < this._rows.length; j++)
			for ( var i = 0; i < this._rows[j].tabCount; i++)
				tabs.push(this._rows[j]
						.removeChild(this._rows[j].childNodes[0]));
		this._tabZone.innerHTML = "";
		this._rows = [];
		var t = this._lastActive;
		this._lastActive = null;
		this._createRow();
		var row = 0;
		...
		for (i; i < tabs.length; i++)
			if ((size + tabs[i]._size + this._a.margin) < limit) {
				...
				if (full && size < prev_size)
					correct_size(this, i - 1);
				this._createRow();
				i--;
				row++;
				prev_size = size;
		...
		;
		if (full && size < prev_size && prev_size != Infinity)
			correct_size(this, i - 1);
		for ( var j = 0; j < this._rows.length; j++)
			this._setTabSizes(this._rows[j]);
		this._setSizes();
		if (this._lastActive = t)
...
if (!window.dhtmlXContainer) {
	window.dhtmlXContainer = function(obj) {
		...
		this.obj.adjustContent = function(parentObj, offsetTop, marginTop,
			...
			;
			this.dhxcont.style.height = px
					+ (marginTop != null ? marginTop : 0) + "px";
			if (notCalcWidth == true) {
			} else {
				if (this.dhxcont.offsetWidth > parentObj.clientWidth) {
					this.dhxcont.style.width = Math.max(0,
							parentObj.clientWidth * 2
									- this.dhxcont.offsetWidth)
							+ "px"
				}
			}
			;
			if (this.dhxcont.offsetHeight > parentObj.clientHeight - offsetTop) {
				var px = (parentObj.clientHeight - offsetTop) * 2
						- this.dhxcont.offsetHeight;
				if (px < 0) {
					px = 0
				}
				;
			...
			if (offsetBottom) {
				if (!isNaN(offsetBottom))
					this.dhxcont.style.height = parseInt(this.dhxcont.style.height)
							- offsetBottom + "px"
			}
			;
			...
			if (notCalcWidth == true) {
				...
				this.dhxcont.mainCont.style.width = this.dhxcont.clientWidth
						+ "px"
			}
			;
			var menuOffset = (this.menu != null ? (!this.menuHidden ? this.menuHeight
					: 0)
					: 0);
			var toolbarOffset = (this.toolbar != null ? (!this.toolbarHidden ? this.toolbarHeight
					: 0)
					: 0);
			var statusOffset = (this.sb != null ? (!this.sbHidden ? this.sbHeight
					: 0)
					: 0);
			this.dhxcont.mainCont.style.height = this.dhxcont.clientHeight
					- menuOffset - toolbarOffset - statusOffset + "px"
		};
		...
		this.obj._attachContent = function(type, obj, append) {
			if (append !== true) {
				while (that.dhxcont.mainCont.childNodes.length > 0) {
					that.dhxcont.mainCont
							.removeChild(that.dhxcont.mainCont.childNodes[0])
				}
			}
			;
			if (type == "url") {
