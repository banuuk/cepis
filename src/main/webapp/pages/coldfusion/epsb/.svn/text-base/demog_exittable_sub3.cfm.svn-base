<cf_page title="Completer Students - Gender Pie Chart">
			...
			<cfloop query="QExitData">
				<cfif #repYear# NEQ "ALL">
					<cfquery name="QTot" datasource="DataReview">
						SELECT count(*) as T, ProgramName, Reportyear FROM tblUkEPSBExitData
				...
				<cfif #repYear# NEQ "ALL">			
					<cfquery name="QMale" datasource="DataReview">
						SELECT count(*) as M FROM tblUkEPSBExitData
						WHERE Reportyear = "#repYear#" AND ProgramName = "#QExitData.ProgramName#" AND Gender = "M"
						GROUP BY ProgramName
					</cfquery>
				<cfelseif #repYear# EQ "ALL">
					<cfquery name="QMale" datasource="DataReview">
						SELECT count(*) as M FROM tblUkEPSBExitData
						WHERE ProgramName = "#QExitData.ProgramName#" AND Gender = "M"
						GROUP BY ProgramName
					</cfquery>
				</cfif>
				...
				<cfif #repYear# NEQ "ALL">
					<cfquery name="QFemale" datasource="DataReview">
						SELECT count(*) as F FROM tblUkEPSBExitData
						WHERE Reportyear = "#repYear#" AND ProgramName = "#QExitData.ProgramName#"  AND Gender = "F"
						GROUP BY ProgramName			
					</cfquery>
				<cfelseif #repYear# EQ "ALL">
					<cfquery name="QFemale" datasource="DataReview">
						SELECT count(*) as F FROM tblUkEPSBExitData
						WHERE ProgramName = "#QExitData.ProgramName#"  AND Gender = "F"
						GROUP BY ProgramName			
					</cfquery>
				</cfif>
				...
				<cfif #QTot.recordcount# GT 0>
					...
					<td>
						...
						<cfchart format="jpg" fontBold="yes" fontSize="16" fontitalic="yes" chartheight="250" chartwidth="250">
