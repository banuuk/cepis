<cf_page title="Admission Students - GPA Line Chart">
	...
	<div>
		...
		<cfquery name="QAdmData" datasource="DataReview">
			...
			from tblUkEPSBAdmData order by ProgramName
		</cfquery>	
			...
			<cfloop query="QAdmData">
				<cfif #repYear# NEQ "ALL">	
					<cfquery name="QData" datasource="DataReview">
						select count(*) as Total , avg(GPA) as Avg, max(GPA) as Max, Min(GPA) as Min, ProgramName, Reportyear 
						from tblUkEPSBAdmData 
				...
				<cfif #repYear# NEQ "ALL">	
					<cfquery name="QGPA" datasource="Datareview">
						Select GPA as Xaxis, count(*) as Yaxis from tblUkEPSBAdmData
						WHERE Reportyear = "#repYear#"  AND ProgramName = "#QAdmData.ProgramName#"
						Group By GPA
					</cfquery>
				<cfelseif #repYear# EQ "ALL">
				...
				<cfif #QData.recordcount# GT 0>
					<tr><td>
						...
						<cfchart font="arial" backgroundcolor="##FFFFCC" databackgroundcolor="##CCCCCC" foregroundcolor="##3366FF" fontSize="14" fontitalic="yes" chartheight="250" chartwidth="800">
