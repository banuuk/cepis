...
<cfparam name="ATTRIBUTES.action" type="string" default="#application.webroot#/customtags/display_members_advising.cfm">
...
<cfform name="display_members" action="#ATTRIBUTES.action#">
	<label for="members" class="label">WORKING SET(only Students) : #getSrchName.SEARCH_NAME#</label>
	<table>
		<tr>
		 <td>
		 ...
		 <select name="add_results" class="select_dropdown" size="#Max(2,Min(20, session.searchResults.RecordCount))#" onchange="changeMember()">
			 ...
			 <cfquery datasource="DataReview" name="QCheckIfStu">
					select UID as stuUID
					from tblCollegeprofile 
					where UID = <cfqueryparam value=#uid#> AND Collpersontypecode = 'STU';
			</cfquery>
			<cfif #QCheckIfStu.recordcount# NEQ 0>
