...
<script type="text/javascript">
	document.title = "CEPIS TEP";
	var global_title = document.getElementById('global_title');
	global_title.innerHTML = "CEPIS TEP";
</script>
			...
			<h:form>
				<h:panelGroup styleClass="wsCore" layout="block">
					...
					<h:selectOneListbox id="wsid"
						...
						actionListener="#{tepController.updateSelectedUser}"
						limitToList="true" reRender="tep_panel"/>
					</h:selectOneListbox>										
				</h:panelGroup>	
			</h:form>
		</h:panelGroup>	
		<h:panelGroup id="tep_panel" layout="block">		
			<rich:tabPanel switchType="ajax" immediate="true" >
				...
				<authz:authorize ifAnyGranted="ROLE_DEVELOPER,ROLE_ADMINISTRATOR,ROLE_STUDENT_ACCOUNT_MANAGER_VIEW">
					<rich:tab label="View" onclick="#{tepController.setUpViewTab}">
						<a4j:include viewId="view-tep.jsp" />
					</rich:tab>
				</authz:authorize>
				
				<authz:authorize ifAnyGranted="ROLE_DEVELOPER,ROLE_ADMINISTRATOR,ROLE_STUDENT_ACCOUNT_MANAGER_ADD_EDIT">
					<rich:tab label="Add" onclick="#{tepController.setUpAddTab}" >
						<a4j:include viewId="add-tep.jsp" />
					</rich:tab>
				</authz:authorize>
				
				<authz:authorize ifAnyGranted="ROLE_DEVELOPER,ROLE_ADMINISTRATOR,ROLE_STUDENT_ACCOUNT_MANAGER_ADD_EDIT">
					<rich:tab label="Update" onclick="#{tepController.setUpEditTab}">
						<a4j:include viewId="edit-tep.jsp" />
					</rich:tab>
				</authz:authorize>
				
				<authz:authorize ifAnyGranted="ROLE_DEVELOPER,ROLE_ADMINISTRATOR,ROLE_STUDENT_ACCOUNT_MANAGER_DELETE">
					<rich:tab label="Delete" onclick="#{tepController.setUpDeleteTab}">
						<a4j:include viewId="delete-tep.jsp" />
					</rich:tab>
				</authz:authorize>
				
				<authz:authorize ifAnyGranted="ROLE_DEVELOPER,ROLE_ADMINISTRATOR,ROLE_STUDENT_ACCOUNT_MANAGER_VIEW">
					<rich:tab label="Report" onclick="#{tepController.setUpReport}">
						<a4j:include viewId="report-tep.jsp" />
					</rich:tab>
				</authz:authorize>
