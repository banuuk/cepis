...
<link href="${facesContext.externalContext.requestContextPath}/css/template.css"
	media="screen" rel="stylesheet" type="text/css"/>
<f:view>
	<h:message for="ukterm_manager">
	</h:message>
	<h:form id="ukterm_manager">
		<br />
		...
		<h:panelGroup id="body">
			<t:dataTable id="scrollList" styleClass="scrollTable"
				...
				var="ukterm" value="#{cepisGlobalComponentConfigurator.ukTermList}"
				preserveDataModel="false" rows="#{cepisConfigurationManager.rowCount}">
				<h:column>
					<f:facet name="header">
						<h:outputText value="#CODE" />
					</f:facet>
					<h:outputText value="#{ukterm.ukTermCode}"></h:outputText>
				</h:column>
				<h:column>
					<f:facet name="header">
						<h:outputText value="UKTerm name" />
					</f:facet>
					<h:outputText value="#{ukterm.shortDesc}"></h:outputText>
				</h:column>
				<h:column>
					<f:facet name="header">
						<h:outputText value="UKTerm Description" />
					</f:facet>
					<h:outputText value="#{ukterm.description}"></h:outputText>
				</h:column>
				<h:column>
					<f:facet name="header">
						<h:outputText value="Status" />
					</f:facet>
					<h:outputText value="#{ukterm.status}"></h:outputText>
				</h:column>
			</t:dataTable>
		</h:panelGroup>
		<h:panelGrid id="scrollers" columns="1" styleClass="scrollerTable2"
			...
			<t:dataScroller id="scroll_0" for="scrollList"
				...
				<h:outputFormat
					value="{0} terms available, displaying {1} terms, from {2} to {3}. Page {4} / {5}"
					styleClass="standard">
